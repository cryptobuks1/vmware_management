...
class FrontendController extends Controller
{
    public function frontIndex()
    {
        ...
        $calen = Plan::latest()->get();
        return view('front.index',compact('pt','service', 'test', 'team','withdraw', 'faq', 'plan','method','level',
            'howitwork', 'latest_deposit', 'latest_withdaw', 'all_plan','calen'));
    }
    ...
    public function contactMailSend(Request $request)
    {
        ...
        send_email( $gnl->email, $request->name, "Contact Us Mail", $request->message.' And My Email:'.$request->email);
    ...
    public function resetLink($code)
    {
        ...
        if ( $reset->status == 1)
        {
            ...
            $pt = "Reset Password";
            return view('auth.passwords.reset', compact('reset','pt'));
        }
